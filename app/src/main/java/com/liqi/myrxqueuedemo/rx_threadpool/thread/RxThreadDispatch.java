package com.liqi.myrxqueuedemo.rx_threadpool.thread;
...
public class RxThreadDispatch extends Thread {
    ...
    public void run() {
        while (isRunTag) {
            isRunState = true;
           // Log.e("运行次数前mRunSize", mRunSize + "<<<<数据源总长度：>>>>"+mList.size());

            //开启多少条线程并发
            tag:while (!isRunSize && mRunSize-1 >= 0) {
                mRunSize= mRunSize-1;
              //  Log.e("运行次数后mRunSize", mRunSize + "");
                ...
                if (!mList.isEmpty()) {
                    ...
                    if (runDisposeSize >= size) {
                        ...
                        isRunState = false;
                        isRunSize = true;
                      //  Log.e("线程休眠01", "已经运行：" + runDisposeSize + "总数据源：" + size);
                        if (null != mOnRunDataDisListener) {
